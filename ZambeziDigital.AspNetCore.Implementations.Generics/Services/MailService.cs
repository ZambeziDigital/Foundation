...
// using ZambeziDigital.AspNetCore.MailManager.Models;
using SmtpClient = MailKit.Net.Smtp.SmtpClient;

namespace ZambeziDigital.AspNetCore.Implementations.Generics.Services;
public interface IMailService : IMailService<BaseResult>;
public class MailService(IServiceScopeFactory scopeFactory) : IMailService
{
    ...
    // [HttpPost]
    public async Task<BaseResult> SendMail(MailRequest mailRequest)
    {
        try
        {
        ...
        catch (Exception e)
        {
            ...
            Console.WriteLine("Error sending mail");
            Console.WriteLine(e.Message);
            return new BaseResult() { Succeeded = false, Errors = new List<string> { e.Message } };
        }
    }

    public async Task<BaseResult> SendMail(MailRequest mailRequest, bool retires = false, int maxRetries = 3, int delay = 1000)
    {
        throw new NotImplementedException();
    }
}
//
// public class MailService(IServiceScopeFactory scopeFactory)
//     : MailService<BaseResult>(scopeFactory), IMailService<BaseResult>;
