    ...
    where TKey : IEquatable<TKey>
{
    ...
    public Task<BaseResult<IQueryable<T>>> SearchAsQueryableAsync(string query, bool paged = false, int page = 0, int pageSize = 10, bool cached = false,
...
public abstract class BaseAuthService<T, TKey>(IServiceScopeFactory serviceScopeFactory) : AuthenticationStateProvider, IBaseService<T, TKey>

    where T : class, IHasKey<TKey>, new()
    where TKey : IEquatable<TKey>
{
    private AuthenticationStateProvider authenticationStateProviderImplementation;
    public List<T> Objects { get; set; } = new();
